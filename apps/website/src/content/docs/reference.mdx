...
XSAF is ESM-only. Import core runtime symbols and contracts from `xsaf`; use published subpaths for bundled adapters.

## Published imports

| Import               | Main exports                                                                                                                                             |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `xsaf`               | `xsaf`, `XsafBuilder`, `XsafAgent`, `HonoBackbone`, `XsaiModelAdapter`, memory and scheduler implementations, `parseCron`, public contracts, tool errors |
| `xsaf/channel/http`  | default `http`, `HttpChannel`                                                                                                                            |
| `xsaf/channel/mock`  | default `mock`, `MockChannel`                                                                                                                            |
| `xsaf/model/mock`    | default `mockModel`, `MockModelAdapter`                                                                                                                  |
| `xsaf/mcp`           | default `mcp`                                                                                                                                            |
| `xsaf/sandbox/local` | default `local`                                                                                                                                          |
| `xsaf/sandbox/host`  | default `host`, `HostSandbox`                                                                                                                            |

Source-internal imports such as `xsaf/core/agent` and `xsaf/memory/in-memory` are not public exports.
...
Runtime use through `builder` requires successful startup. `.asAgent()` returns a sealed agent for reuse and delegation.
