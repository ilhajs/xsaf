...
const researcher = xsaf
  ...
  .asAgent("researcher", "Research one focused question");
```

The name must be lowercase snake_case and shares the parent's model-visible namespace with local tools. The description defaults to `Delegate a task to <name>` when omitted.
