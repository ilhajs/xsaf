...
```ts
import type { StandardJSONSchemaV1, StandardSchemaV1, XsafToolSchema } from "xsaf";
...
`agent.ask(prompt, schema)` accepts a `StandardSchemaV1` output validator. It does not require `XsafToolSchema` because XSAF does not publish that schema as a model-visible tool definition.
...
```ts
const builder = xsaf.agent(config).sandbox(productionSandbox).tool(tool);
```
...
const builder = xsaf.agent(config).sandbox(local());
```

`local()` executes the function in the current JavaScript process. It provides **no filesystem, network, shell, memory, or process isolation**. A timeout only aborts the signal cooperatively; it cannot forcibly terminate arbitrary host code.

<Aside type="danger" title="Do not mistake local execution for a sandbox">

Use `xsaf/sandbox/local` only when host-process execution is acceptable. Register an isolation driver for untrusted code or privileged operations.

</Aside>

### Compatibility host adapter

```ts
import host from "xsaf/sandbox/host";

const unsafe = host({ allowUnsafeHostExecution: true });
```

This compatibility adapter also runs in the host process and requires the literal acknowledgement. Its `fs`, `network`, and `shell` permission fields are descriptive and are not enforcement.
...
- [Choose a sandbox](/tools#sandbox-security)
- [MCP](/mcp)
