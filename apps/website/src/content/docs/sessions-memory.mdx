...
| `/mcp`         | Mounted only after `.serve({ transport: "http" })`.                    |
| `/chat`        | Default route of `xsaf/channel/http` when that channel is registered.  |
...
```ts
const agent = builder.asAgent("http_agent");
...
```ts
import http from "xsaf/channel/http";

const channel = http({ name: "web", path: "/chat" });
const builder = xsaf.agent(config).channel(channel);
```
...
String and object results return JSON. Streaming results return Server-Sent Events with one `data` event per chunk. A missing session ID is generated with `crypto.randomUUID()`.

The HTTP channel tracks pending responses by session in FIFO order. Its `send()` target must be a session ID, and closing the channel rejects pending requests. Use `xsaf/channel/mock` for deterministic in-process tests; see [Testing](/testing).
...
```ts
import { InMemoryMemory, inMemory } from "xsaf";

const memory = inMemory();
const another = new InMemoryMemory();
```
...
```ts
const builder = xsaf.agent(config).memory(durableMemory);
```
