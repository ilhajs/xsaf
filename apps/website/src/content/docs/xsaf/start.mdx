...
```ts
const agent = xsaf.agent(config).memory(memory).channel(channel);
...
A started agent forwards [`.invoke()`](/xsaf/invoke), [`.invoke()`](/xsaf/invoke), [`.ask()`](/xsaf/ask), and [`.fetch()`](/xsaf/fetch) to its agent. Calling those methods through the agent before startup throws.
