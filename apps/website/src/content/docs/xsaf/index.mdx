---
title: agent()
description: API reference for creating and configuring XSAF agents with the fluent agent factory.
sidebar:
  order: 2
  label: agent()
---

Import the `agent` factory from `@xsaf/agent`. Calling [`agent()`](/xsaf/agent) creates an `XsafAgent`; every configuration method returns that same agent so configuration reads as one chain.

```ts
import { agent } from "@xsaf/agent";

const bot = agent(config).memory(memory).sandbox(sandbox).tool(searchTool).channel(channel).serve();
