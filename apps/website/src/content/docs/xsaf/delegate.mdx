...
```ts
builder.delegate(agent: XsafAgent, options?: DelegateOptions): XsafBuilder
```
...
Create reusable children with [`.asAgent()`](/xsaf/as-agent), and see [Tools & Security](/recipes/tools) for shared execution policy.
